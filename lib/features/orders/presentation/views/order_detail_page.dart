...
class _OrderDetailPageEnhancedState
  ...
  // ---------- Responsive helpers ----------
  double _width = 400;
  bool get _isMobile => _width < 850;
  bool get _isNarrowMobile => _width < 420;

  double _rFont(double size) {
    // base scale: reduce font on mobile slightly, enlarge on large screens
    if (_isNarrowMobile) return size * 0.9;
    if (_isMobile) return size * 0.95;
    if (_width > 1200) return size * 1.05;
    return size;
  }

  double _horizontalPadding() => _isMobile ? 12 : 24;
  double _cardPadding() => _isMobile ? 12 : 20;
  double _appBarHeight() => _isMobile ? 56 : 64;
  ...
  Widget _buildProductItem(OrderProductEntity product) {
    ...
    final String? imageUrl = info['imageUrl'];
    final double sousTotal = productPrice * product.quantite;
    final imageSize = _isMobile ? 56.0 : 72.0;
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Prix: ${productPrice.toStringAsFixed(0)} FCFA',
                      style: TextStyle(
                        fontSize: _rFont(12),
                        color: Colors.grey.shade600,
                      ),
  ...
  // ---------- Responsive layouts ----------
  Widget _buildMobileLayout(OrderEntity order) {
            ...
            child: Column(
              children: [
                ListView.separated(
                  ...
                  itemBuilder: (context, index) =>
                      _buildProductItem(order.produits[index]),
                ),
                const Divider(height: 1),
                SizedBox(height: 8),
                _buildPriceRow(
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        ...
                        label: Text(
                          'Imprimer',
                          style: TextStyle(fontSize: _rFont(14)),
                        ),
                        style: OutlinedButton.styleFrom(
              ...
              children: [
                ...
                if (order.statut.toLowerCase() == 'en cours')
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton.icon(
                      onPressed: () => _updateStatus(order.id!, 'validé'),
                      icon: const Icon(Icons.check_circle),
                      ...
                      style: ElevatedButton.styleFrom(
                        ...
                        foregroundColor: Colors.white,
                        padding:
                        EdgeInsets.symmetric(vertical: _isMobile ? 12 : 14),
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(8),
                        ),
                      ),
                ...
                if (order.statut.toLowerCase() == 'validé')
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton.icon(
                      onPressed: () => _updateStatus(order.id!, 'terminé'),
                      icon: const Icon(Icons.done_all),
                ...
                if (order.surPlace)
                  _buildInfoRow(
                      ...
                      value: order.numeroTable != null
                          ? 'Table ${order.numeroTable}'
                          : 'Oui'),
                if (order.livraison) Divider(height: 24),
                if (order.livraison)
                  _buildInfoRow(
                    icon: Icons.delivery_dining,
                    label: 'Livraison',
  ...
  Widget _buildDesktopLayout(OrderEntity order) {
    // Two-column layout similar to original, with responsive paddings/font sizes
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Expanded(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return _buildProductItem(order.produits[index]);
                        },
                      ),
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        ...
                        style: OutlinedButton.styleFrom(
                          padding:
                          EdgeInsets.symmetric(vertical: _isMobile ? 12 : 16),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(8),
                          ),
                        ),
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      // Loading / error handling (kept similar to original but with responsive AppBar)
      if (orderState.isLoading) {
        return Scaffold(
          backgroundColor: Colors.grey.shade50,
          appBar: AppBar(
            ...
            leading: IconButton(
              icon: Icon(Icons.arrow_back, color: Colors.black, size: _isMobile ? 20 : 24),
              onPressed: () => context.pop(),
            ),
            title: Text(
              'Commande ...',
      ...
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              icon: Icon(Icons.edit, color: AppColors.darkBlue, size: _isMobile ? 20 : 24),
              onPressed: () {
                context.push('/orders/edit/${order.id}');
              },
              tooltip: 'Modifier',
            ...
            IconButton(
              icon: Icon(Icons.delete, color: AppColors.primaryRed, size: _isMobile ? 20 : 24),
              onPressed: _showDeleteConfirmation,
              tooltip: 'Supprimer',
            ),
        ...
        body: updateStatusState.isLoading
          ...
          child: isMobile ? _buildMobileLayout(order) : _buildDesktopLayout(order),
        ),
      );
    });
  }
